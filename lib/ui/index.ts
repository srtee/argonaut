...
// Helper function to unescape HTML entities (reverse of escapeHtmlStr)
function unescapeHtmlStr(text) {
    if (!text) return '';
...
/**
 * Reset all papers and return to initial state
 */
...
import { initLoadDOM, loadPapers, loadFromUrlParam } from './load.js';
...
import { showLoadState, initVisibilityDOM } from './visibility.js';
...
export function initEventListeners() {
    ...
    if (papersList) {
        papersList.addEventListener('click', (e) => {
            ...
            if (editBtn) {
                e.stopPropagation();
                const key = unescapeHtmlStr(editBtn.dataset.key);
    ...
    if (papersList) {
        papersList.addEventListener('blur', (e) => {
            if (e.target.classList.contains('comments')) {
                const key = unescapeHtmlStr(e.target.dataset.key);
