...
export async function exportJSON() {
    ...
    try {
        // Get checkbox values
        const bibInfo = includeBibInfo ? includeBibInfo.checked : false;
        const abstracts = includeAbstracts ? includeAbstracts.checked : false;
        console.log('[Export] includeBibInfo:', bibInfo, 'includeAbstracts:', abstracts);
        ...
        // Update filename based on what's included
        if (bibInfo && abstracts) {
            filename = 'papers.json';
        } else if (bibInfo) {
            filename = 'papers-with-bibinfo.json';
        } else if (abstracts) {
            filename = 'papers-with-abstracts.json';
        } else {
...
export async function exportBibTeX() {
    ...
    try {
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            const key = paper._key || doi;
            if (status) {
                status.textContent = `Fetching BibTeX ${i + 1} of ${entries.length}: ${key}`;
            }
            await new Promise(resolve => setTimeout(resolve, 0));

            // Use DOI from the key for new format, or _doi for legacy format
            const paperDoi = paper._doi || doi;
            if (paperDoi) {
                let bibtex = await fetchBibTeX(paperDoi);
                if (bibtex) {
                    ...
                    if (!pages) {
                        ...
                        pages = await fetchPagesFromCrossref(paperDoi);
                    }
...
export async function exportBibTeXTagged() {
    ...
    try {
        ...
        const entries = Object.entries(state.papersData).filter(([doi, paper]) => {
