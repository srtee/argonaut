...
export function initEventListeners() {
    ...
    document.addEventListener('keydown', async (e) => {
        if (e.key === 'Escape' && state.currentEditingKey !== null) {
            const { hasUnsavedChanges, closeTagDialog } = await import('./ui/tagEditor.js');
    ...
    document.addEventListener('click', (e) => {
        if (state.currentEditingKey !== null) {
            ...
            if (editor && !editor.contains(e.target)) {
                import('./ui/tagEditor.js').then(({ hasUnsavedChanges, closeTagDialog }) => {
                    if (hasUnsavedChanges()) {
                        if (confirm('You have unsaved changes. Discard them?')) {
                            closeTagDialog();
                            import('./papers.js').then(({ applyTagFilter }) => {
    ...
    if (papersList) {
        papersList.addEventListener('blur', (e) => {
            if (e.target.classList.contains('comments')) {
                ...
                import('./ui/comments.js').then(({ saveComment }) => {
