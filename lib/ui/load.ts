...
import { showPapersState, showLoadingState } from './visibility.js';
...
/**
 * Check for jsonurl parameter in URL and load if present
 * @returns {Promise<boolean>} - True if JSON was loaded from URL
 */
export async function loadFromUrlParam() {
    const urlParams = new URLSearchParams(window.location.search);
    const jsonUrl = urlParams.get('jsonurl');

    if (jsonUrl && jsonUrl.trim()) {
        console.log('[Load] Found jsonurl parameter:', jsonUrl);
        try {
            // Load from URL
            const data = await loadFromUrl(jsonUrl.trim());

            // Process and display papers
            const { processPapers } = await import('../papers.js');
            const convertedData = convertToDoiIndexed(data);
            const processedPapers = await processPapers(convertedData);
            const { renderPapers } = await import('../papers.js');
            renderPapers(processedPapers);

            // Switch to papers view
            showPapersState();
            showStatus(`Loaded ${processedPapers.length} papers from URL`);
            setTimeout(hideStatus, 8000);

            return true;
        } catch (err) {
            console.error('[Load] Error loading from jsonurl parameter:', err);
