...
export function hasUnsavedChanges() {
    const key = state.currentEditingKey;
...
export function openTagDialog(key) {
    ...
    const paper = state.papersData[doi];
    if (!paper) {
        console.error('Paper not found for DOI:', doi);
        return;
    }

    // Reset state
    store.set({
        currentEditingDoi: doi,
        tentativeTags: [],
    ...
    let card = document.querySelector(`.paper[data-key="${escapedKey}"]`);
    if (!card) {
        card = document.querySelector(`.paper-card[data-key="${escapedKey}"]`);
    }
    if (!card) {
...
function renderInlineTagEditor(tagsContainer) {
    ...
    if (state.tentativeTags.length === 0 && state.tentativeTagsRemoved.length === 0) {
        ...
        state.tentativeTags.forEach(tag => {
            tagsHtml += `<button type="button" class="tag-editor__item" data-tag="${escapeHtmlStr(tag)}" aria-label="Remove tag: ${escapeHtmlStr(tag)}">${escapeHtmlStr(tag)} <span class="tag-editor__remove">×</span></button>`;
        });
        // Render tentatively removed tags
        state.tentativeTagsRemoved.forEach(tag => {
            tagsHtml += `<button type="button" class="tag-editor__item tag-editor__item--removed" data-tag="${escapeHtmlStr(tag)}" aria-label="Restore tag: ${escapeHtmlStr(tag)}">${escapeHtmlStr(tag)} <span class="tag-editor__restore">+</span></button>`;
        });
    }
...
export function closeTagDialog({ restoreContent = true } = {}) {
    ...
    const escapedEditingKey = CSS.escape(state.currentEditingKey);
    let card = document.querySelector(`.paper[data-key="${escapedEditingKey}"]`);
    if (!card) {
        card = document.querySelector(`.paper-card[data-key="${escapedEditingKey}"]`);
    }
    if (card) {
...
function saveTagChanges() {
    ...
    const doi = findDoiByKey(state.currentEditingKey);
    if (!doi) return;
