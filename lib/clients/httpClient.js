...
export async function httpClient(url, options = {}) {
    ...
    // For Crossref, omit credentials and don't set mode (let browser handle CORS)
    const credentials = url.includes('api.crossref.org') ? 'omit'
        : (shouldIncludeCredentials ? 'include' : 'same-origin');

    const defaultOptions = {
        credentials,
        mode: url.includes('api.crossref.org') ? 'cors' : undefined,
        ...restOptions,
    };
...
export async function jsonRequest(url, options = {}) {
    // Build headers - only Accept header for Crossref (no User-Agent, no Content-Type)
    const headers = {
