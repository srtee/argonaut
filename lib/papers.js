...
export async function addPaperByDoi() {
    ...
    try {
        ...
        // Add to state.papersData (including abstract)
        store.set({
            papersData: {
                ...state.papersData,
                [key]: {
                ...
                for (const ref of referencesWithDoi) {
                    ...
                    try {
                        ...
                        if (refBibtex) {
                            ...
                            if (state.papersData[refKey]) {
                                console.log(`[Papers] Skipping duplicate reference: ${refKey}`);
                                continue;
                            }

                            // Add reference to state (including abstract)
                            store.set({
                                papersData: {
                                    ...state.papersData,
                                    [refKey]: {
...
export function createPaperCard(key, paperData, bibInfo, abstract) {
    ...
    const comments = `<textarea class="comments" placeholder="Add your notes..." data-key="${escapeHtmlStr(key)}" aria-label="Notes for this paper">${escapeHtmlStr(paperData._comments || '')}</textarea>`;
    ...
    card.innerHTML = `
        <div class="paper__header">
            <h3 class="paper__title">
                <button class="key-edit-btn" aria-label="Edit paper key" type="button" data-key="${escapeHtmlStr(key)}">
            ...
            <div class="key-editor" data-key="${escapeHtmlStr(key)}">
        ...
        ${comments}
        <div class="tags">
            <button class="tag-edit-btn" aria-label="Edit tags" type="button" data-key="${escapeHtmlStr(key)}">
    ...
    keySaveBtn.addEventListener('click', async (e) => {
        ...
        // Update also-read references in all papers (now in papersData only)
...
export async function processPapers(data, options = {}) {
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        showStatus(`Fetching ${i + 1} of ${entries.length}: ${key}`);
        ...
        if (useCache) {
            ...
            if (cached && cached.bibInfo) {
                ...
                if (paper._doi && (!hasAllBibInfo || !hasAbstract)) {
                    console.log('[Papers] Cache incomplete, fetching fresh data for:', key, { hasAllBibInfo, hasAbstract });
...
export function applyTagFilter() {
    ...
    pagePapers.forEach(([key, paper]) => {
        const bibInfo = { title: paper.title || key, ...paper };
        const card = createPaperCard(key, paper, bibInfo, paper.abstract);
