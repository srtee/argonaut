...
export function createPaperCard(key, paperData, bibInfo, abstract) {
    ...
    const abstractContent = abstract ? `<div class="abstract-content">${escapeHtmlStr(abstract)}</div>` : '<p class="no-abstract">No abstract available</p>';
    ...
    if (escapeHtmlStr(bibInfo.journal)) {
        citationParts.push(`<span class="journal">${escapeHtmlStr(bibInfo.journal)}</span>`);
    }
    if (bibInfo.year) {
    ...
    card.innerHTML = `
        <div class="paper-header">
            <h3 class="paper-title">${escapeHtmlStr(bibInfo.title || key)}</h3>
        </div>
        ...
        ${alsoread ? `<div class="alsoread-container" role="group" aria-label="Also read papers"><span class="alsoread-label">Also read:</span> ${alsoread}</div>` : ''}
