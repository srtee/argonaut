    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        if (useCache) {
            ...
            if (cached && cached.bibInfo) {
                ...
                // Check if cache is complete (has pages if DOI present)
                const hasPagesInCache = cached.bibInfo && cached.bibInfo.pages;
                const hasAbstractInCache = cached.abstract;

                // If we have DOI but cache is missing pages, need to refetch from Crossref
                if (paper._doi && !hasPagesInCache) {
                    console.log('[Papers] Cache missing pages, fetching fresh data for:', key);
