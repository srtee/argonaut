...
export async function initGitHubAuth() {
    ...
    if (sessionId && authStatus === 'success') {
        ...
        if (state.papersData && Object.keys(state.papersData).length > 0) {
            console.log('[GitHub Auth] Papers found in app state, displaying them');
...
export async function restorePapersState() {
    ...
    console.log('[GitHub Auth] restorePapersState called, savedState:', savedState ? 'found' : 'not found');
    if (savedState) {
        try {
            const saved = JSON.parse(savedState);
            console.log('[GitHub Auth] Restoring state:', saved);

            if (saved.hasPapers) {
                // Restore the papers data and state
                if (saved.papersData) {
                    console.log('[GitHub Auth] Restoring papersData:', Object.keys(saved.papersData));
