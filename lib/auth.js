...
import { restoreVisibilityState, getVisibilityState } from './ui/visibility.js';
...
export async function initGitHubAuth() {
    ...
    if (sessionId && authStatus === 'success') {
        ...
        // Check if papers exist in main app state and prompt user
        // This handles the case where page was refreshed
        if (state.papersData && Object.keys(state.papersData).length > 0) {
            const hasPapers = Object.keys(state.papersData).length;
            const confirmed = confirm(`You have ${hasPapers} paper(s) saved from your previous session. Would you like to restore them?`);
            if (confirmed) {
                const { displayPapers } = await import('./papers.js');
                await displayPapers();
            } else {
                // Clear the saved state if user declines
