...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!loadedFromUrl) {
        ...
        let hasStoredPapers = false;
        if (storedData) {
            try {
                const data = JSON.parse(storedData);
                hasStoredPapers = data && Object.keys(data).length > 0;
            } catch (e) {}
        }

        if (hasStoredPapers) {
            // Prompt BEFORE loading
            const confirmed = confirm(`You have ${Object.keys(JSON.parse(storedData)).length} paper(s) saved from your previous session. Would you like to restore them?`);
