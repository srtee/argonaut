...
export function initDOM() {
    ...
    saveMethodFile = document.getElementById('saveMethodFile');
    saveMethodStorage = document.getElementById('saveMethodStorage');
    saveMethodGist = document.getElementById('saveMethodGist');
...
export function initEventListeners() {
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && onboardingModal.classList.contains('onboarding--active')) {
            hideOnboarding();
        }
    });
...
export function openTagDialog(key) {
    ...
    const card = document.querySelector(`.paper[data-key="${key}"]`);
    if (!card) {
