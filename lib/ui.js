...
export function toggleTag(tag) {
    ...
    const { applyTagFilter } = await import('./papers.js');
    applyTagFilter();
...
export function updateInputOptionUI(selectedValue) {
    ...
    if (selectedOption) {
        ...
        if (selectedValue === 'gist' && getSessionId()) {
            ...
            const { updateGistVisibility } = await import('./auth.js');
...
export function updateSaveOptionUI(selectedValue) {
    ...
    if (selectedOption) {
        ...
        // Load gist options if "gist" is selected and user is authenticated
        if (selectedValue === 'gist' && getSessionId()) {
            const { loadGistOptionsForSaveSelector } = await import('./github.js');
            loadGistOptionsForSaveSelector();
            const { updateSaveGistVisibility } = await import('./auth.js');
